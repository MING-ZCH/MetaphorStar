...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, path: str):
        ...
        def robust_save(obj, path, retries=10, sleep_time=10):
            for i in range(retries):
                try:
                    torch.save(obj, path)
                    return
                except RuntimeError as e:
                    print(f"[rank-{self.rank}] Failed to save {path} (attempt {i+1}/{retries}). Error: {e}")
                    time.sleep(sleep_time)
            # Last attempt
            torch.save(obj, path)

        robust_save(model_state_dict, model_path)
        robust_save(optim_state_dict, optim_path)
        robust_save(extra_state_dict, extra_path)
