...
def timer(name: str, timing_raw: Dict[str, float]):
    ...
    try:
        ...
        # Ensure timing is recorded even if an exception occurs
        elapsed = time_module.time() - start_time
        if elapsed > 0 and not (isinstance(elapsed, float) and elapsed != elapsed):  # Check for NaN
            timing_raw[name] = elapsed
        else:
            timing_raw[name] = 0.0
